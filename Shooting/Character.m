...
-(id) init{
    if( self = [super init]){
        ...
        skin.position = ccp(side.position.x,side.position.y - side.contentSize.height/2);
...
-(void) moveCharacter:(CGPoint)position{
    ...
    //상 하 이동 체크.
    if((side.position.y + side.contentSize.height/2 + position.y <= 450 && position.y > 0) || (side.position.y - side.contentSize.height/2 + position.y >= 0 && position.y < 0))
        side.position = ccp(side.position.x, side.position.y + position.y);
    skin.position = ccp(side.position.x,side.position.y - side.contentSize.height/2);  //c1 position
    [self moveTarget];
}
-(void) moveTarget{
...
/*    if([attackArray containsObject:[NSNumber numberWithInt:enemy.enemyID]]){
        ...
        NSLog(@"HP %d",hp);
        if(hp<= 0){
            [UserInfo sharedUserInfo].result = YES;
 ...
 */
    if(isHitting == YES){
        ...
        [skin runAction:[CCSequence actions:[CCBlink actionWithDuration:1 blinks:3],[CCBlink actionWithDuration:1 blinks:5],[CCBlink actionWithDuration:0.5 blinks:5],[CCCallFunc actionWithTarget:self selector:@selector(endHitting)], nil]];
    }
    [UserInfo sharedUserInfo].hp = hp;
}
-(void) endHitting{
